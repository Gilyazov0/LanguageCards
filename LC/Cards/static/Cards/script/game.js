...
class Abstract_game extends Widget {
  constructor(owner, tags, user_tags, FAs, container = undefined) {
    ...
    this.settings.add_setting(
      new SelectInput(this, this.FAs[0], "Front side:", this.FAs),
      "front_attribute"
    );
    this.settings.add_setting(
      new NumericInput(this, 30, "Answer time:"),
      "answer_time"
    );

    let captions = {
      include: "Add cards with tags:",
      exclude: "except cards with tags:",
    };
    this.settings.add_setting(
      new Tag_selector_set(
        this,
        undefined,
        this.user_tags,
        this.tags,
    ...
    this.settings.tag_selectors_set.onChange = function () {
  ...
  _show_settings() {
    this.settings.set_container(
      this.container.querySelector("#settings_container")
    );
    this.settings.show();

    this.container.querySelector("#start-game-btn").onclick = function (e) {
      this.start();
    }.bind(this);
  ...
  _show_game() {
    fetch("/Cards/get_cards", {
      ...
      .then((result) => {
        this.card_set = new Card_set(
          this,
          result.cards,
          result.tags,
          [this.settings.front_attribute.value],
          this.back_attributes
        );
  ...
  _is_answered(answer_key = undefined) {
    ...
    if (new_answer_key == undefined)
      new_answer_key = this._get_current_answer_key();
  ...
  _save_answer(is_right, answer) {
    ...
    this.user_answers[this._get_current_answer_key()] = {
      is_right: is_right,
      answer: answer,
    };
  }
  ...
  _update_selected_settings() {
    ...
    this.back_attributes = back_attributes.filter((value) => {
      return value != front_attribute;
    });
  ...
  _getHTML() {
    if (this._state.is_in_state(State.settings))
      return this._getHTML_settings();
  ...
  _getHTML_settings() {
    let result = "";
    result = ` 
            <div class="alert alert-primary" id="label-cards-count" >
  ...
  _update_settings_page() {
    fetch("/Cards/get_cards_count", {
      ...
      .then((result) => {
        ...
        const label = this.container.querySelector("#label-cards-count");
        if (label != null) {
          if (card_count == 0) {
            label.innerHTML = `<h1>No cards found</h1>`;
            label.className = "alert alert-warning";
          } else {
            label.innerHTML = `<h1>Selected ${card_count} cards</h1>`;
...
class Simple_game extends Abstract_game {
  constructor(owner, tags, user_tags, FAs, container = undefined) {
    ...
    this.settings.add_setting(
      new BooleanInput(this, true, "Show score:"),
      "display_score"
    );
  }
  ...
  _show_game() {
    ...
    this.container.querySelector("#show-prev-card-btn").onclick =
      this._show_new_card.bind(this, "left");
    this.container.querySelector("#show-next-card-btn").onclick =
      this._show_new_card.bind(this, "right");
    this.container.querySelector("#reverse-card-btn").onclick = function () {
      this.active_card_obj.reverse(true);
    }.bind(this);
    this.container.querySelector("#results_right").onclick =
      this._show_results.bind(this, "results_right");
    this.container.querySelector("#results_wrong").onclick =
      this._show_results.bind(this, "results_wrong");
  ...
  _getHTML_game() {
    ...
    const game = `
        ...
        <div id ="game_controls">
            <div id = "additional_controls"> </div>
  ...
  _update_game_page(show_card = true) {
    ...
    if (card_set.cards_count() == 0) {
      ...
      this.container.querySelector("#show-prev-card-btn").disabled =
        card_set.get_current_card_number() <= 0;
      this.container.querySelector("#show-next-card-btn").disabled =
        card_set.get_current_card_number() >= card_set.cards_count() - 1;
  ...
  _show_game_or_results() {
    ...
    for (let i = 0; i < game_elements.length; i++) {
      this.container.querySelector(`#${game_elements[i]}`).style.display =
        game_elements_state;
    }

    for (let i = 0; i < result_elements.length; i++) {
      this.container.querySelector(`#${result_elements[i]}`).style.display =
        result_elements_state;
    }
    this.container
      .querySelector("#results_right")
      .classList.remove("border", "border-white", "border-lg");
    this.container
      .querySelector("#results_wrong")
      .classList.remove("border", "border-white", "border-lg");
    if (this.state.is_in_state(State.results)) {
      this.container
        .querySelector("#" + this.state.name)
        .classList.add("border", "border-white", "border-lg");
      ...
      for (let i = 0; i < this.card_set.cards_count(); i++) {
        ...
        if (
          (answer_is_right && this._state.is_in_state(State.results_right)) ||
      ...
      this.container
        .querySelector("#get_result_cards")
  ...
  _new_active_card_obj() {
    this.active_card_obj = this.card_set.get_card(
      this.card_set.get_current_card_number(),
      this.active_card_container
    );
...
class Simple_score_game extends Simple_game {
  ...
  _show_game() {
    if (this.settings.display_score.value) {
      let additional_controls = this.container.querySelector(
        "#additional_controls"
      );
      additional_controls.innerHTML =
        this._getHTML_simple_score_game_controls();
...
class Variants_game extends Simple_game {
  constructor(owner, tags, user_tags, FAs, container = undefined) {
    ...
    this.settings.add_setting(
      new NumericInput(this, 4, "Number of variants:"),
      "number_of_variants"
    );

    this.settings.add_setting(
      new SelectInput(this, this.FAs[1], "Answer type:", this.FAs),
      "answer_attribute"
    );

    this.settings.add_setting(
      new LabelSetting(this, "Select cards to pick answers from"),
      "variants_tag_selector_label"
    );
  ...
  _show_game() {
    ...
    additional_controls.innerHTML = this._getHTML_variants_game_controls();

    for (let i = 0; i < this.settings.number_of_variants.value; i++) {
      this.container.querySelector(`#answer_button_${i}`).onclick = function (
        event
      ) {
    ...
    fetch("/Cards/get_cards", {
      ...
      .then((response) => response.json())
      .then((result) => {
        this.variants_card_set = new Card_set(
          this,
          result.cards,
          result.tags,
          [this.settings.front_attribute.value],
  ...
  _create_answer_variants() {
    let _get_random_variant = function () {
      ...
      //10 attempts max to find card with defined attribute to avoid
      for (let i = 0; i < 10; i++) {
        let variant_id =
          this.variants_card_set.order[
            generateRandomInteger(this.variants_card_set.order.length) - 1
          ];
        variant = this.variants_card_set.get_card_FA_value(
          variant_id,
          this.settings.answer_attribute.value
        );
        if (variant) break;
      }
      return variant;
    ...
    for (let id in this.card_set.cards) {
      ...
      let right_variant_place =
        generateRandomInteger(this.settings.number_of_variants.value) - 1;
      let right_variant = this.card_set.get_card_FA_value(
        id,
        this.settings.answer_attribute.value
      );
  ...
  _getHTML_variants_game_controls() {
    ...
    for (let i = 0; i < this.settings.number_of_variants.value; i += 2) {
      let variant2 = "";
      if (i < this.settings.number_of_variants.value - 1) {
        variant2 = `
                <div style="width:calc(50% - 5px)">
                    <button id="answer_button_${
                      i + 1
                    }" class="form-control btn btn-primary">answer</button>
                </div>`;
      }
  ...
  _check_answer() {
    let is_right = false;
    let right_answer = this.active_card_obj.get_FA_value(
      this.settings.answer_attribute.value
    );
    ...
    if (is_right) this._play_sound("/static/Cards/sounds/answer right.mp3");
...
class Print_game extends Simple_game {
  ...
  _check_answer() {
    ...
    if (
      ...
      this.active_card_obj
        .get_FA_value(this.settings.answer_attribute.value)
        ?.toLowerCase()
    )
      is_right = true;
    else is_right = false;
  ...
  _show_game() {
    ...
    additional_controls.innerHTML = this._getHTML_print_game_controls();
    ...
    answer.onkeyup = function (event) {
      if (event.key === "Enter") this._check_answer();
    }.bind(this);
    return super._show_game();
  }
}
...
class MetaGame extends Widget {
  constructor(container) {
    ...
    fetch("/Cards/get_metadata", {
      ...
      .then((result) => {
        ...
        this.games = [
          new Variants_game(this, tags, user_tags, FAs),
          new Simple_score_game(this, tags, user_tags, FAs),
          new Print_game(this, tags, user_tags, FAs),
        ];
        let game_list = [];
        for (let i = 0; i < this.games.length; i++) {
          this.games[i].onChange = this._on_game_change.bind(this);
          this.settings.aggregate(this.games[i].settings);
  ...
  show() {
    ...
    this.active_game.set_container(
      this.container.querySelector("#game_container")
    );
    // this if needed to avoid several fetch requests from game
  ...
  _show_settings() {
    ...
    //     }.bind(this);
    this.settings.set_container(
      this.container.querySelector("#meta_game_settings")
    );
    this.settings.show();
  }
  ...
  _getHTML() {
    let result =
      '<div id="meta_game_settings" style="padding:5px"></div><div id="game_container" style="padding:5px"></div>';
...
document.addEventListener("DOMContentLoaded", function () {
  const meta_game = new MetaGame(
    document.querySelector("#meta_game_container")
  );
