...
export class BooleanInput extends SimpleInput {
    ...
    _getHTML(){
        return  ` 
          <div class="form-group group-gp">
              <label class="form-control-lg label-gp" for="input_id">${this.lable}</label>
              <input id='input_id' type="${this.type}" class="form-control input-gp" checked = ${this._value}/>           
          </div>               
          `
      }
}
...
export class Settings_ extends Widget{
    /**
    * @constructor
    * @param {Object} owner
    * @param {HTMLElement} container  
    */
        ...
        show(){
            ...
            this._settings.forEach(element => {
                element.set_container(this.container.querySelector('#' + element.name))
                element.show()
            });
