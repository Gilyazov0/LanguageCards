...
export class State {
   ...
   * @param {State} specified_state parent state which is specify by this State.
   ...
   *
   * @param {string} name
  //  * @param {State} specified parent state which is specify by this State.
   *     For example State "Move" could be specified by "Run" or "Walk"
   * @returns
   */
...
export class Saveable {
  ...
  save_to_JSON() {
    let result = {};
    for (let key in this) {
      if (this._do_not_save.includes(key)) continue;
  ...
  /** saves attribute 'attribute_name' to
   * @protected
   * @param {string} attribute_name
   * @param {JSON} objectJSON
   */
 ...
 *   - remember its 'owner' - any object which owns the Widget
...
export class Widget extends Saveable {
   ...
   * @returns {boolean} true - successfully showed, false not
   */
  show() {
    if (this.container) {
  ...
  _CheckAction() {
    ...
    if (Math.abs(d.x) > Math.abs(d.y)) {
      if (Math.abs(d.x) > sensitivity) {
        if (d.x > 0) this._swipe_left();
        else this._swipe_right();
      }
    } else {
      if (Math.abs(d.y) > sensitivity) {
        if (d.y > 0) this._swipe_up();
        else this._swipe_down();
      }
    }
  }
}
...
export class Timer extends Widget {
   ...
   * @param {Object} owner
   * @param {HTMLElement} container
   * @param {Number} initial_time initial number of seconds
   */
  constructor(
    owner,
    initial_time,
  ...
  /**
   * if running updates Timers value
   */
  _tic() {
    if (this.state.is_in_state(State.running)) {
      this.value =
        this._initial_time +
        Math.round((this._start_time - new Date().getTime()) / 1000);
      if (this.value <= 0) {
        this.value = 0;
        this.stop();
      }
  ...
  _calculate_font_size() {
    ...
    const w_font_size =
      (this.container.scrollWidth / String(this._initial_time).length) *
      2 *
      0.9;
  ...
  _getHTML() {
    ...
    if (font_size == undefined) {
      font_size = this._calculate_font_size();
    }
...
export class Tag_selector extends Widget {
  ...
  show() {
    ...
    $(this.container).on(
      "click",
      ".tag-selector",
      this._form_check_input_onClick.bind(this)
    );
    return true;
  }
  ...
  _update_selected_tags_dom() {
    const tag_selectors = this.container.querySelectorAll(
    ...
    for (let i = 0; i < tag_selectors.length; i++) {
      if (tag_selectors[i].checked) {
        this.selected_tags.push(
          Number(tag_selectors[i].getAttribute("tag_id_data"))
        );
      }
    }
...
export class Card extends Widget {
   ...
   * @param {Array.<string>} front front attributes of this card
   * @param {Array.<string>} back back attributes of this card
   * @param {Array.<Tag>} user_tags all user tags available
   * @param {HTMLElement} container
   * @returns {Card}
   */
  static create_without_card_set(
    owner,
    card_data,
    front,
    back,
   ...
   * @param {Object} owner
   * @param {string} card_data_JSON {Card_data} serialized to JSON string
   * @param {string} tags_data_JSON {Array.Tag} serialized to JSON string
   * @param {HTMLElement} container
   * @returns {Card}
   */
  static create_from_JSON_strings(
    owner,
    card_data_JSON,
    tags_data_JSON,
    container
    ...
    const back_ = FAs_;
    return Card.create_without_card_set(
      owner,
      card_data_,
      front_,
      back_,
  ...
  move(in_out, direction, onAnimationendExternal = undefined) {
    let onAnimationend = function (event) {
      ...
      if (onAnimationendExternal) onAnimationendExternal();
    };
  ...
  show() {
    ...
    for (let i = 0; i < card_titles.length; i++) {
      ...
      card_titles[i].onclick = function (event) {
        event.target.card.reverse(true);
      };
    }
  ...
  _tags_to_string(tags, class_ = "") {
    ...
    for (let i = 0; i < tags.length; i++) {
      result += `<span card_id = '${this.get_id()}' tag_id = '${
        tags[i].id
      }' class = '${class_}'>&lt;${tags[i].name}&gt;</span>`;
    }
    return result;
  }
  ...
  _getHTML() {
    ...
    for (let i = 0; i < sides.length; i++) {
      ...
      // user tags applied to this card
      ...
      if (this.card_set != null) {
        card_number_label = `${
          this.card_set.get_card_number(this) + 1
        }/${this.card_set.cards_count()}`;
      } else {
      ...
      let user_tags_string = ` <div style="display: flex; align-items: center;">                                 
                                        ${this._tags_to_string(
                                          card_user_tags,
                                          "tag_user card_tag_item"
                                        )} 
                                        ${user_tags_selector}
                                    </div>
                                  `;
      ...
      result += `
            <div class="card border-success mb-3 game-card ${
              i == 0 ? "flip-card-front" : "flip-card-back"
            }">
...
export class Card_set extends Saveable {
  ...
  change_card(increment) {
    ...
    new_card_number =
      new_card_number > this.cards_count() - 1
        ? this.cards_count() - 1
        : new_card_number;
...
/**
 * Abstract class for user setting.
 *  - process user input
 *  - show itself inside DOM container
 *  - save itself to serializable object
 */
export class Setting extends Widget {
  constructor(owner, initial_value, container = undefined) {
    ...
    this.name = undefined;
    this.visible = true;
    //if not undefined override value set by user
...
/**
 * Abstract Setting for base HTML input types
 */
export class SimpleInput extends Setting {
  constructor(owner, initial_value, label, container = undefined) {
    super(owner, initial_value, container);
    this.label = label;
    this.type = undefined;
  }
  _getHTML() {
    return ` 
        <div class="form-group group-gp">
            <label class="form-control-lg label-gp" for="input_id">${this.label}</label>
...
export class NumericInput extends SimpleInput {
  constructor(owner, initial_value, label, container = undefined) {
    super(owner, initial_value, label, container);
...
export class SelectInput extends SimpleInput {
  constructor(owner, initial_value, label, options, container = undefined) {
    super(owner, initial_value, label, container);
  ...
  _getHTML() {
    let result = `
        <div class="form-group  group-gp"> <label class="form-control-lg label-gp">${this.label}</label>
            <select id ="input_id" class="form-control-lg form-select form-select-lg input-gp flex-grow-1">
               `;
    for (let i = 0; i < this._options.length; i++) {
      let selected =
        this._options[i] == this._value ? 'selected="selected"' : "";
...
/**
 * Setting to pick two sets of tags 'include' and 'exclude'
 */
export class Tag_selector_set extends Setting {
   ...
   * @type {Object}
   * @property {string} include caption for tag selectors
   ...
   * @param {Array.<Tag>} user_tags [{'name':name,'id': id},{}...]
   * @param {Array.<Tag>} tags [{'name':name,'id': id},{}...]
   * @param {TS_captions} captions  captions = {'include':'caption for includes', 'exclude':'caption for excludes' }
   */
  constructor(
    owner,
    initial_value,
  ...
  add_tag_selectors_pair() {
    let ts_pair = {};
    ts_pair["include"] = new Tag_selector(
      this,
      this.user_tags,
      this.tags,
    ...
    ts_pair["exclude"] = new Tag_selector(
      this,
      this.user_tags,
      this.tags,
      this.captions["exclude"]
    );
  ...
  show() {
    ...
    this.container.querySelector("#add-tag-selectors-btn").onclick =
      this.add_tag_selectors_pair.bind(this);
  }
...
export class Settings extends Widget {
  ...
  add_setting(setting, name) {
    ...
    else this._settings.push(setting);
